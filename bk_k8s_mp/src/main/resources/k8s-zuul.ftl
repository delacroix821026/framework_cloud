...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: ${project.name}
        image: nexusserver:${nexusPort}/${project.build.finalName}:${project.version}
        imagePullPolicy: IfNotPresent
        lifecycle:
          postStart:
            exec:
              #command: ["/bin/sh", "-c", "ls /home/DockerCommand/ > /aa"]
        ...
        - name: SPRING_PROFILES_ACTIVE
          value: ${profileConvert}
        - name: DEVLOPER_NAME
          value: "${DEVLOPER_NAME}-${profile}"
        - name: fw.onServer.enabled
          value: "true"
        - name: SERVER_PORT
          value: "5001"
